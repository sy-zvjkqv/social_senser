...
for i in range(0, len(list_mobile)):
    ...
    sns.stripplot(x="Tweets_num", y="Population", data=df_asa, ax=ax1_1)
    x_axis = []
    for i in range(0, max(df_asa["Tweets_num"]) + 1):
        x_axis.append(i)
    ...
    sns.stripplot(x="Tweets_num", y="Population", data=df_hiru, ax=ax1_2)
    x_axis = []
    ...
    sns.stripplot(x="Tweets_num", y="Population", data=df_ban, ax=ax1_3)
    x_axis = []
    ...
    save_PATH = (
        "/home/is/shuntaro-o/dev/compare_population_and_tweet_number/outputs/swarm/24hour/"
        + name_key
        + ".png"
    )
