...
for i in range(0, len(list_mobile)):
    ...
    df_asa = pd.DataFrame(data = np.stack([tweets_asa_flatten, mobile_asa_flatten]).T, columns=['Tweets_num', 'Population'])
    df_hiru = pd.DataFrame(data = np.stack([tweets_hiru_flatten, mobile_hiru_flatten]).T, columns=['Tweets_num', 'Population'])
    df_ban = pd.DataFrame(data = np.stack([tweets_ban_flatten, mobile_ban_flatten]).T, columns=['Tweets_num', 'Population'])
